...
// 1) إكمال أي دفعة A2U عالقة قبل إنشاء جديدة (وإلا ترفض Pi كل ما بعدها)
async function recoverIncomplete(pi) {
    const list = await pi.getIncompleteServerPayments();
    for (const p of list) {
        try {
            let txid = p.transaction?.txid;
            if (!txid) txid = await pi.submitPayment(p.identifier);   // لم تُرسل بعد → آمن
            await pi.completePayment(p.identifier, txid);
            const orderId = p.metadata?.orderId;
            if (orderId) {
                await db.collection('orders').doc(orderId).set({
                    status: 'released', payoutPaymentId: p.identifier, payoutTxid: txid,
                    releasedAt: admin.firestore.FieldValue.serverTimestamp(), recovered: true
                }, { merge: true });
            }
...
// 2) حجز الطلب ذرّياً: held → releasing
async function claimOrder(orderId, { requireBuyer } = {}) {
    ...
    return db.runTransaction(async t => {
        ...
        const o = snap.data();
        if (requireBuyer && o.buyerUsername !== requireBuyer)
...
async function releaseOrder(orderId, opts = {}) {
    ...
    try {
        paymentId = await pi.createPayment({
            amount: payoutAmount,  // استخدم المبلغ بعد خصم الرسوم
            memo: `Arabeka payout ${orderId.slice(0, 8)}`,
            metadata: { 
                orderId, 
                type: opts.auto ? 'order_release_auto' : 'order_release',
        ...
        await ref.update({ status: 'held', payoutAttempts: admin.firestore.FieldValue.increment(1),
                           lastPayoutError: JSON.stringify(e.response?.data || e.message) });
        throw e;
    }

    // من هنا فصاعداً لا نُرجع الحالة إلى held أبداً — recoverIncomplete هو من يُكمل
    ...
    return { payoutPaymentId: paymentId, txid };
}

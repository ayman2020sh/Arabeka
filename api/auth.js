...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const userData = {
            piUid: me.uid,
            ownerUid: me.username,
            lastLoginAt: admin.firestore.FieldValue.serverTimestamp()
        };

        // حفظ عنوان المحفظة إن منح المستخدم إذن wallet_address
        const wallet = me.wallet_address || me.walletAddress || null;
        if (wallet) userData.walletAddress = wallet;

        await db.collection('users').doc(me.username).set(userData, { merge: true });
